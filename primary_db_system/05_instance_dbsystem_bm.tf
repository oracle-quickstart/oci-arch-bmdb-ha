...
resource "oci_database_db_system" "primary-db-bm" {
  ...
  subnet_id = "${oci_core_subnet.primary-public-subnet1.id}"
...
# Get DB node list
data "oci_database_db_nodes" "primary-bm" {
  compartment_id = "${var.compartment_ocid}"
  db_system_id = "${oci_database_db_system.primary-db-bm.id}"
}

# Get DB node details
data "oci_database_db_node" "primary-bm" {
  db_node_id = "${lookup(data.oci_database_db_nodes.primary-bm.db_nodes[0], "id")}"
}

# Gets the OCID of the first (default) vNIC
data "oci_core_vnic" "primary-bm" {
  vnic_id = "${data.oci_database_db_node.primary-bm.vnic_id}"
}

resource "null_resource" "primary-bm" {
  provisioner "file" {
    connection {
	agent = false
	timeout = "10m"
	host = "${data.oci_core_vnic.primary-bm.public_ip_address}"
	user = "opc"
...
output "Public IP of the DB system-BM" {
  value = ["${data.oci_core_vnic.primary-bm.public_ip_address}"]
}
